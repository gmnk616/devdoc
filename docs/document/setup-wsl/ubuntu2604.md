...
### **<font color="DodgerBlue">`Ubuntu-26.04`をインストール</font>**
...
インストールコマンドを実施します。  
(インストールコマンドを実行すると、`Ubuntu 26.04 LTS`のインストール&ダウンロードを実施します)  
...
エクスポート前に`Ubuntu-26.04`ディストリビューションが起動している場合は停止させます  
...
```powershell title="powershell"
wsl --export Ubuntu-26.04 $env:USERPROFILE/winenv/wsl/ubuntu2604/ansible_test.tar
```
...
:::tip
wslディストリビューション(`Ubuntu-26.04`)がインストールされている場合は、アンインストールします
...
`Ubuntu-26.04`(ansible_test)をインポートします

```powershell title="powershell"
wsl --import Ubuntu-26.04 $env:USERPROFILE/winenv/wsl/ubuntu2604/ $env:USERPROFILE/winenv/wsl/ubuntu2604/ansible_test.tar
```
...
`ansible`が`sudo-rs`に未対応のため、以下インストールを実施（対応すれば不要かも）

```bash
sudo apt install sudo
```

さらに`sudo-rs`から`sudo`に切り替え

```bash
sudo update-alternatives --set sudo /usr/bin/sudo.ws
```

以下コマンドで`sudo`に切り替わってるか確認可能
...
```bash
git clone https://github.com/gmnk616/setup-wslenv.git && cd ./setup-wslenv/ubuntu2604
```
...
### **<font color="DodgerBlue">`Ubuntu-26.04`をエクスポート</font>**
...
```powershell title="powershell"
wsl --export Ubuntu-26.04 $env:USERPROFILE/winenv/wsl/ubuntu2604/ubuntu26_04.tar
```

## インポート

### **<font color="DodgerBlue">`Ubuntu-26.04`をインポート</font>**
...
`Ubuntu-26.04`をインポートします

```powershell title="powershell"
wsl --import Ubuntu-26.04 $env:USERPROFILE/winenv/wsl/ubuntu2604/ $env:USERPROFILE/winenv/wsl/ubuntu2604/ubuntu26_04.tar
```

`Ubuntu-26.04`をデフォルトディストリビューションに設定します
